/**
 * Autogenerated by Thrift Compiler (0.9.3)
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */
#include "MetricDefs_types.h"

#include <algorithm>
#include <ostream>

#include <thrift/TToString.h>

namespace palo {


TMetricDef::~TMetricDef() throw() {
}


void TMetricDef::__set_key(const std::string& val) {
  this->key = val;
__isset.key = true;
}

void TMetricDef::__set_kind(const  ::palo::TMetricKind::type val) {
  this->kind = val;
__isset.kind = true;
}

void TMetricDef::__set_units(const  ::palo::TUnit::type val) {
  this->units = val;
__isset.units = true;
}

void TMetricDef::__set_contexts(const std::vector<std::string> & val) {
  this->contexts = val;
__isset.contexts = true;
}

void TMetricDef::__set_label(const std::string& val) {
  this->label = val;
__isset.label = true;
}

void TMetricDef::__set_description(const std::string& val) {
  this->description = val;
__isset.description = true;
}

uint32_t TMetricDef::read(::apache::thrift::protocol::TProtocol* iprot) {

  apache::thrift::protocol::TInputRecursionTracker tracker(*iprot);
  uint32_t xfer = 0;
  std::string fname;
  ::apache::thrift::protocol::TType ftype;
  int16_t fid;

  xfer += iprot->readStructBegin(fname);

  using ::apache::thrift::protocol::TProtocolException;


  while (true)
  {
    xfer += iprot->readFieldBegin(fname, ftype, fid);
    if (ftype == ::apache::thrift::protocol::T_STOP) {
      break;
    }
    switch (fid)
    {
      case 1:
        if (ftype == ::apache::thrift::protocol::T_STRING) {
          xfer += iprot->readString(this->key);
          this->__isset.key = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 2:
        if (ftype == ::apache::thrift::protocol::T_I32) {
          int32_t ecast0;
          xfer += iprot->readI32(ecast0);
          this->kind = ( ::palo::TMetricKind::type)ecast0;
          this->__isset.kind = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 3:
        if (ftype == ::apache::thrift::protocol::T_I32) {
          int32_t ecast1;
          xfer += iprot->readI32(ecast1);
          this->units = ( ::palo::TUnit::type)ecast1;
          this->__isset.units = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 4:
        if (ftype == ::apache::thrift::protocol::T_LIST) {
          {
            this->contexts.clear();
            uint32_t _size2;
            ::apache::thrift::protocol::TType _etype5;
            xfer += iprot->readListBegin(_etype5, _size2);
            this->contexts.resize(_size2);
            uint32_t _i6;
            for (_i6 = 0; _i6 < _size2; ++_i6)
            {
              xfer += iprot->readString(this->contexts[_i6]);
            }
            xfer += iprot->readListEnd();
          }
          this->__isset.contexts = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 5:
        if (ftype == ::apache::thrift::protocol::T_STRING) {
          xfer += iprot->readString(this->label);
          this->__isset.label = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      case 6:
        if (ftype == ::apache::thrift::protocol::T_STRING) {
          xfer += iprot->readString(this->description);
          this->__isset.description = true;
        } else {
          xfer += iprot->skip(ftype);
        }
        break;
      default:
        xfer += iprot->skip(ftype);
        break;
    }
    xfer += iprot->readFieldEnd();
  }

  xfer += iprot->readStructEnd();

  return xfer;
}

uint32_t TMetricDef::write(::apache::thrift::protocol::TProtocol* oprot) const {
  uint32_t xfer = 0;
  apache::thrift::protocol::TOutputRecursionTracker tracker(*oprot);
  xfer += oprot->writeStructBegin("TMetricDef");

  if (this->__isset.key) {
    xfer += oprot->writeFieldBegin("key", ::apache::thrift::protocol::T_STRING, 1);
    xfer += oprot->writeString(this->key);
    xfer += oprot->writeFieldEnd();
  }
  if (this->__isset.kind) {
    xfer += oprot->writeFieldBegin("kind", ::apache::thrift::protocol::T_I32, 2);
    xfer += oprot->writeI32((int32_t)this->kind);
    xfer += oprot->writeFieldEnd();
  }
  if (this->__isset.units) {
    xfer += oprot->writeFieldBegin("units", ::apache::thrift::protocol::T_I32, 3);
    xfer += oprot->writeI32((int32_t)this->units);
    xfer += oprot->writeFieldEnd();
  }
  if (this->__isset.contexts) {
    xfer += oprot->writeFieldBegin("contexts", ::apache::thrift::protocol::T_LIST, 4);
    {
      xfer += oprot->writeListBegin(::apache::thrift::protocol::T_STRING, static_cast<uint32_t>(this->contexts.size()));
      std::vector<std::string> ::const_iterator _iter7;
      for (_iter7 = this->contexts.begin(); _iter7 != this->contexts.end(); ++_iter7)
      {
        xfer += oprot->writeString((*_iter7));
      }
      xfer += oprot->writeListEnd();
    }
    xfer += oprot->writeFieldEnd();
  }
  if (this->__isset.label) {
    xfer += oprot->writeFieldBegin("label", ::apache::thrift::protocol::T_STRING, 5);
    xfer += oprot->writeString(this->label);
    xfer += oprot->writeFieldEnd();
  }
  if (this->__isset.description) {
    xfer += oprot->writeFieldBegin("description", ::apache::thrift::protocol::T_STRING, 6);
    xfer += oprot->writeString(this->description);
    xfer += oprot->writeFieldEnd();
  }
  xfer += oprot->writeFieldStop();
  xfer += oprot->writeStructEnd();
  return xfer;
}

void swap(TMetricDef &a, TMetricDef &b) {
  using ::std::swap;
  swap(a.key, b.key);
  swap(a.kind, b.kind);
  swap(a.units, b.units);
  swap(a.contexts, b.contexts);
  swap(a.label, b.label);
  swap(a.description, b.description);
  swap(a.__isset, b.__isset);
}

TMetricDef::TMetricDef(const TMetricDef& other8) {
  key = other8.key;
  kind = other8.kind;
  units = other8.units;
  contexts = other8.contexts;
  label = other8.label;
  description = other8.description;
  __isset = other8.__isset;
}
TMetricDef& TMetricDef::operator=(const TMetricDef& other9) {
  key = other9.key;
  kind = other9.kind;
  units = other9.units;
  contexts = other9.contexts;
  label = other9.label;
  description = other9.description;
  __isset = other9.__isset;
  return *this;
}
void TMetricDef::printTo(std::ostream& out) const {
  using ::apache::thrift::to_string;
  out << "TMetricDef(";
  out << "key="; (__isset.key ? (out << to_string(key)) : (out << "<null>"));
  out << ", " << "kind="; (__isset.kind ? (out << to_string(kind)) : (out << "<null>"));
  out << ", " << "units="; (__isset.units ? (out << to_string(units)) : (out << "<null>"));
  out << ", " << "contexts="; (__isset.contexts ? (out << to_string(contexts)) : (out << "<null>"));
  out << ", " << "label="; (__isset.label ? (out << to_string(label)) : (out << "<null>"));
  out << ", " << "description="; (__isset.description ? (out << to_string(description)) : (out << "<null>"));
  out << ")";
}

} // namespace
