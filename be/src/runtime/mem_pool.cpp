...
void MemPool::acquire_data(MemPool* src, bool keep_current) {
    ...
    _chunks.insert(insert_chunk, src->_chunks.begin(), end_chunk);    //jungle comment : insert ChunkInfo to _chunks , ChunkInfo has data pointer ,if free it  ,need to acquire_data ?
