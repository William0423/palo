...
OLAPStatus CumulativeHandler::init(SmartOLAPTable table) {
    ...
    if (_old_cumulative_layer_point == -1) {
        OLAP_LOG_INFO("tablet has an unreasonable cumulative layer point. "
                      "[tablet='%s' cumulative_layer_point=%d]",
                      _table->full_name().c_str(), _old_cumulative_layer_point);
    ...
    _obtain_header_rdlock();
    res = _check_whether_satisfy_policy();
    _release_header_lock();
    if (res != OLAP_SUCCESS) {
        _table->release_cumulative_lock();
        return res;
    ...
    res = _calculate_need_merged_versions();
    _release_header_lock();
    if (res != OLAP_SUCCESS) {
        _table->release_cumulative_lock();
...
OLAPStatus CumulativeHandler::run() {
    ...
    do {
        ...
        _new_cumulative_index = new (nothrow) OLAPIndex(_table.get(),
                                                        _cumulative_version,
...
OLAPStatus CumulativeHandler::_check_whether_satisfy_policy() {
    ...
    if (res != OLAP_SUCCESS) {
        OLAP_LOG_TRACE("get delta versions failed when check cumulative policy. [res=%d]", res);
        return res;
    }
...
OLAPStatus CumulativeHandler::_calculate_need_merged_versions() {
    ...
    while (index < delta_number) {
        ...
        for (; index < delta_number; ++index) {
            ...
            size_t delta_size = _table->get_version_entity_by_version(delta).data_size;
        ...
        if (need_merged_versions.size() == 1 || total_size == 0) {
            ...
            if (index < delta_number
                    && _table->get_version_entity_by_version(delta_versions[index]).data_size >=
        ...
        // 如果有多个可合并文件，则可以进行cumulative expansion的合并过程
        // 如果只有只有一个可合并的文件，为了效率，不触发cumulative expansion的合并过程
        if (need_merged_versions.size() != 1) {
    ...
    // 如果不设置新的cumulative_layer_point, 则下次执行cumulative expansion时，扫描的文件和这次
    // 扫描的文件相同，依然找不到可以合并的delta文件, 无法执行合并过程。
    // 依此类推，就进入了死循环状态，永远不会进行cumulative expansion
...
bool CumulativeHandler::_find_previous_version(const Version current_version,
    ...
    for (Versions::const_iterator version = all_versions.begin();
        ...
        if (version->second == current_version.first - 1) {
            ...
            size_t data_size = _table->get_version_entity_by_version(*version).data_size;
...
OLAPStatus CumulativeHandler::_do_cumulative_expansion() {
    ...
    res = merger.merge(_data_source, true, &merged_rows, &filted_rows);
    if (res != OLAP_SUCCESS) {
    ...
    if (res != OLAP_SUCCESS) {
        ...
        if (ret != OLAP_SUCCESS) {
            OLAP_LOG_FATAL("roll back failed. [table=%s]",  _table->full_name().c_str());
        }
...
OLAPStatus CumulativeHandler::_update_header(vector<OLAPIndex*>* unused_indices) {
    vector<OLAPIndex*> new_indices;
...
void CumulativeHandler::_delete_unused_delta_files(vector<OLAPIndex*>* unused_indices) {
...
OLAPStatus CumulativeHandler::_roll_back(const vector<OLAPIndex*>& old_olap_indices) {
    ...
    if (res != OLAP_SUCCESS) {
        OLAP_LOG_FATAL("failed to replace data sources. [table=%s]", _table->full_name().c_str());
        return res;
    }

    res = _table->save_header();
    if (res != OLAP_SUCCESS) {
        OLAP_LOG_FATAL("failed to save header. [table=%s]", _table->full_name().c_str());
        return res;
    }
